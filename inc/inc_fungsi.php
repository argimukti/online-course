<?php 
include_once ("inc_koneksi.php");

function url_dasar(){
  //$_SERVER['SERVER_NAME'] : alaamat website contoh.com
  // menyimpan directory website contoh.com/blog
  $protocol = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' || $_SERVER['SERVER_PORT'] == 443) ? "https://" : "http://";
  $url_dasar = $protocol . $_SERVER['SERVER_NAME'] . rtrim(dirname($_SERVER['SCRIPT_NAME']), '/');
  return $url_dasar;

  // $url_dasar = "http://" .$_SERVER["SERVER_NAME"]. dirname($_SERVER['SCRIPT_NAME']);
  // return $url_dasar;
}
function ambil_gambar($id_tulisan){
  global $conn;
  $sqli1 = "SELECT * FROM halaman WHERE id = '$id_tulisan'";
  $q1 = mysqli_query($conn, $sqli1);
  $r1 = mysqli_fetch_array($q1);
  $text = $r1['isi'];

  preg_match('/< *img[^>]*src *= *["\']?([^"\']*)/i', $text, $image);
  $gambar = isset($image[1]) ? $image[1] : '';
  
  if (!empty($gambar)) {
    $gambar = str_replace("../images/", url_dasar() . "/images/", $gambar);
    return $gambar;
  }else{
    return '';
  }
} 

function ambil_kutipan($id_tulisan){
  global $conn;
  $sqli1 = "SELECT * FROM halaman WHERE id = '$id_tulisan'";
  $q1 = mysqli_query($conn, $sqli1);
  $r1 = mysqli_fetch_assoc($q1);
  $text = $r1["kutipan"];
  return $text;
}

function ambil_judul($id_tulisan){
  global $conn;
  $sqli1 = "SELECT * FROM halaman WHERE id = '$id_tulisan'";
  $q1 = mysqli_query($conn, $sqli1);
  $r1 = mysqli_fetch_assoc($q1);
  $text = $r1["judul"];
  return $text;
}

function ambil_isi($id_tulisan){
  global $conn;
  $sqli1 = "SELECT * FROM halaman WHERE id = '$id_tulisan'";
  $q1 = mysqli_query($conn, $sqli1);
  $r1 = mysqli_fetch_assoc($q1);
  $text = strip_tags($r1["isi"] );
  return $text;
}

function judul_bersih($judul){
  $judul_baru = strtolower($judul);
  $judul_baru = preg_replace("/[^a-zZ-Z0-9\s]/","", $judul_baru);
  $judul_baru =str_replace(" ", "-", $judul_baru);
  return $judul_baru;
}

function buat_link_halaman($id){
  global $conn;
  $sqli1 = "SELECT * FROM halaman WHERE id = '$id'";
  $q1 = mysqli_query($conn, $sqli1);
  $r1 = mysqli_fetch_assoc($q1);
  $judul = judul_bersih($r1["judul"] );
  return url_dasar()."/halaman.php/$id/$judul";
}

function dapatkan_id(){
  $id ="";
  if(isset($_SERVER['PATH_INFO'])){
    $id = dirname($_SERVER['PATH_INFO']);
    $id = preg_replace("/[^0-9]/", "" , $id);
  }
  return $id;
}

function set_isi($isi){
  $isi =str_replace("../images/", url_dasar()."/images/",$isi);
  return $isi;
}

function maximum_kata($isi, $maximum){
  $array_isi = explode(" ", $isi);
  $array_isi = array_slice($array_isi, 0 , $maximum);
  $isi = implode(" ", $array_isi);
  return $isi;
}

function tutors_foto($id){
  global $conn;
  $sqli1 = "SELECT * FROM tutors WHERE id = '$id'";
  $q1 = mysqli_query($conn, $sqli1);
  $r1 = mysqli_fetch_array($q1);
  $foto = $r1["foto"];

  if($foto){
    return $foto;

  }else{
    return 'tutors_default_pic.jpg';
  }
}

function buat_link_tutors($id){
  global $conn;
  $sqli1 = "SELECT * FROM tutors WHERE id = '$id'";
  $q1 = mysqli_query($conn, $sqli1);
  $r1 = mysqli_fetch_assoc($q1);
  $nama = judul_bersih($r1["nama"] );
  return url_dasar()."/tutors.php/$id/$nama";
}

function partners_foto($id){
  global $conn;
  $sqli1 = "SELECT * FROM partners WHERE id = '$id'";
  $q1 = mysqli_query($conn, $sqli1);
  $r1 = mysqli_fetch_array($q1);
  $foto = $r1["foto"];

  if($foto){
    return $foto;

  }else{
    return 'partners_default_pic.jpg';
  }
}

function buat_link_partners($id){
  global $conn;
  $sqli1 = "SELECT * FROM partners WHERE id = '$id'";
  $q1 = mysqli_query($conn, $sqli1);
  $r1 = mysqli_fetch_assoc($q1);
  $nama = judul_bersih($r1["nama"] );
  return url_dasar()."/partners.php/$id/$nama";
}

function ambil_isi_info($id, $kolom){
  global $conn;
  $sqli1 = "SELECT $kolom FROM info WHERE id = '$id'";
  $q1 = mysqli_query($conn, $sqli1);
  $r1 = mysqli_fetch_array($q1);
  return $r1[$kolom];
}

use PHPMailer\PHPMailer\PHPMailer;
use PHPMailer\PHPMailer\SMTP;
use PHPMailer\PHPMailer\Exception;
function kirim_email($email_penerima, $nama_penerima, $judul_email, $isi_email){

//Import PHPMailer classes into the global namespace
//These must be at the top of your script, not inside a function
$email_pengirim = "abiko211@gmail.com";
$nama_pengirim = "noreply";

//Load Composer's autoloader
require getcwd().'/vendor/autoload.php';

//Create an instance; passing `true` enables exceptions
$mail = new PHPMailer(true);

try {
    //Server settings
    $mail->SMTPDebug = 0;                      //Enable verbose debug output
    $mail->isSMTP();                                            //Send using SMTP
    $mail->Host       = 'smtp.gmail.com';                     //Set the SMTP server to send through
    $mail->SMTPAuth   = true;                                   //Enable SMTP authentication
    $mail->Username   = $email_pengirim;                     //SMTP username
    $mail->Password   = 'dbiv jkrs xzkm ruyr';                               //SMTP password
    $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS;            //Enable implicit TLS encryption
    $mail->Port       = 465;                                    //TCP port to connect to; use 587 if you have set `SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS`

    //Recipients
    $mail->setFrom($email_pengirim,$nama_pengirim);
    $mail->addAddress($email_penerima, $nama_penerima);     //Add a recipient



    //Content
    $mail->isHTML(true);                                  //Set email format to HTML
    $mail->Subject =$judul_email;
    $mail->Body    = $isi_email;
    

    $mail->send();
    return "Sukses";
} catch (Exception $e) {
    return "Gagal: {$mail->ErrorInfo}";
}
}



?>